...
test('total metric tons is loaded correctly with different dropdown selections', async (t) => {
  let totalCo2Amount = Selector('#metric-one').withText('618')
  ...
  totalCo2Amount = Selector('#metric-one').withText('172')
  await t.expect(totalCo2Amount.exists).ok()
  ...
  totalCo2Amount = Selector('#metric-one').withText('304')
  await t.expect(totalCo2Amount.exists).ok()
  ...
  totalCo2Amount = Selector('#metric-one').withText('276')
  await t.expect(totalCo2Amount.exists).ok()
})
...
test('carbon equivalency component displays each option when clicked', async (t) => {
  ...
  const gallonsOfGas = Selector('p').withText('69,540')
  const treeSeedlings = Selector('p').withText('10,218')
  const milesDriven = Selector('p').withText('1,533,500')
